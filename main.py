import os import threading from io import BytesIO from flask import Flask from PIL import Image from pyrogram import Client, filters from pyrogram.types import ( InlineKeyboardMarkup, InlineKeyboardButton, Message )
...
API_ID = int(os.environ.get("API_ID", 0)) API_HASH = os.environ.get("API_HASH", "") BOT_TOKEN = os.environ.get("BOT_TOKEN", "") FORCE_JOIN = os.environ.get("FORCE_JOIN", "")  # private channel for join request JOIN_CHANNEL = "AniReal_Anime_Zone"  # public channel for Join Here button START_IMG = "https://graph.org/file/5fb2a9e904ae7870b8862-fb58603ada523ac527.jpg"
...
async def is_subscribed(client, user_id): try: member = await client.get_chat_member(FORCE_JOIN, user_id) return member.status in ["member", "administrator", "creator"] except Exception: return False
...
@app.on_callback_query() async def callback_query_handler(client, callback_query): data = callback_query.data if data == "check_subscribe": if await is_subscribed(client, callback_query.from_user.id): await callback_query.message.delete() await start_handler(client, callback_query.message) else: await callback_query.answer("\u274c You're still not joined!", show_alert=True)
...
@app.on_message(filters.photo & filters.private) async def photo_to_sticker(client, message): # Ignore the bot's own image message (like /start image) if message.from_user.is_self: return
...
@app.on_message(filters.command("create_own_bot") & filters.private) async def create_own_bot(client, message): await message.reply_text( "\ud83d\udc4b Hey Dear.. This is an Paid Bot, Create Your Own Bot to Massage Here:- @AniReal_Support" )
